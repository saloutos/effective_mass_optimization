...
% Group them
q   = [  xb;  yb;  thb;  th1;  th2;  th3];     % generalized coordinates
dq  = [ dxb; dyb; dthb; dth1; dth2; dth3];     % first time derivatives
ddq = [ddxb;ddyb;ddthb;ddth1;ddth2;ddth3];     % second time derivatives
...
e1hat =  cos(thb+th1)*ihat + sin(thb+th1)*jhat;
e2hat =  cos(thb+th1+th2)*ihat + sin(thb+th1+th2)*jhat;
e3hat = cos(thb+th1+th2+th3)*ihat + sin(thb+th1+th2+th3)*jhat;
...
% TODO: change topology here?
r_motor1 = rO; % located at base
r_motor2 = rA; 
r_motor3 = rB;
...
omegab = dthb;
omega1 = dthb + dth1;
omega2 = dthb + dth1 + dth2;
omega3 = dthb + dth1 + dth2 + dth3;

% base
Tb = (1/2)*mb*dot(dr_mb,dr_mb) + (1/2)*Ib*omegab^2;
% links
...
% motor bodies...rotate with previous link in this topology
T_m1 = (1/2)*m_motor*dot(dr_motor1,dr_motor1) + (1/2)*I_motor*omegab^2;
...
% motors
Vg_m1 = m_motor*g*dot(r_motor1, khat);
Vg_m2 = m_motor*g*dot(r_motor2, khat);
Vg_m3 = m_motor*g*dot(r_motor3, khat);

T = simplify(Tb + T1 + T2 + T3 + T_m1+ T_m2 + T_m3 + T_mr1 + T_mr2 + T_mr3);
Vg = Vgb + Vg1 + Vg2 + Vg3 + Vg_m1 + Vg_m2 + Vg_m3;
...
Q_tau1R = M2Q(-tau1*khat,omegab*khat);
Q_tau2R = M2Q(-tau2*khat,omega1*khat);
Q_tau3R = M2Q(-tau3*khat,omega2*khat);

Q_tau = (Q_tau1+Q_tau1R)+(Q_tau2+Q_tau2R)+(Q_tau3+Q_tau3R);

Q_F = F2Q([Fx;Fy;0],(rC-rO)); % TODO: check this?

Q = Q_tau + Q_F;

% Assemble the array of cartesian coordinates of the key points
keypoints = [rO(1:2) rA(1:2) rB(1:2) rC(1:2)];
