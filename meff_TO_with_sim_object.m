...
q_data_TO = z_data_TO(1:3,:); % using z_data
dq_data_TO = z_data_TO(4:6,:); % using z_data
...
function plot_arm_object_simulation(t_i,z_i,zo_i,p,p_obj,pts,ee_data,qo_data)
    ...
    obj_pts = plot(qo_data(1,:),qo_data(2,:),'m'); % object trajectory
    
    circ_pts_i = circle_points(zo_i(1), zo_i(2), p_obj(3));
    h_obj = plot(circ_pts_i(1,:), circ_pts_i(2,:),'m','LineWidth',1.5);
    ...
    function circ_pts = circle_points(xc, yc, r)
        angles = linspace(0,2*pi,125);
        circ_pts = [xc+r*cos(angles); yc+r*sin(angles)];
    end
