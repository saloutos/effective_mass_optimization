function [TO_data, opt_time] = meff_optimization(pts, Tf, p, alpha_vec)
    ...
    % modified to have simpler cost function formulation
    ...
    p_sol = zeros(3,num_pts); % xact, yact, error
    v_sol = zeros(3,num_pts); % vxact, vyact, verror
    for ii=1:num_pts
        ...
        p_sol(1:2,ii) = p_temp(1:2);
        p_error = p_sol(1:2,ii) - pts(:,ii);
        p_sol(3,ii) = norm(p_error);
        % fill v_sol
        v_sol(1:2,ii) = ve_i;
        v_error = v_sol(1:2,ii) - vels(:,ii);
        v_sol(3,ii) = norm(v_error);
    end
    
    % save outputs
    TO_data = [time_vec; z_sol; meff_sol; peff_sol; p_sol; v_sol]; % concatenate everything
...
function [c,ceq] = joint_constraints(z,p,time_vec,pts,vels)
    ...
    ceq = [1000*ceq; 100000*ceq_q_0; 100000*ceq_q_f]; % only constrain initial and final positions and velocities
...
function total_cost = meff_cost(z,p,time_vec,pts,vels,alpha_vec)
    ...
    for ii=2:num_pts
        ...
        % effective mass cost
        LLv_inv = LLv_arm_op_inv(z_i,p);
%         meff_cost_vec(ii) = 1/(ev_i'*LLv_inv*ev_i); % inverse is actual effective mass
        meff_cost_vec(ii) = -(ev_i'*LLv_inv*ev_i); % negative seems nicer to work with than inverse
%         meff_cost_vec(ii) = -((ev_i'*LLv_inv*ev_i)^2); % squared?
