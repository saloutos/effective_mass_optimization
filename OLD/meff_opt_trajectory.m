...
center = [0.8, -0.4]; % [0.7, -0.1] produced some weird behavior
radius = 0.2;
...
qsol_vec = zeros(3,num_pts);
opt_time_vec = zeros(1,num_pts);
figure; % open new figure

% save animation as a gif
filename = 'circle_traj_gif_1.gif';
...
for ii=1:num_pts
   ...
   if ii==1
       ...
       [sol_hist_start, sol_hist, sol_opt_time] = meff_opt_single_point_warm_start_func(q,p,v_vec(:,ii),pt_vec(:,ii));
   else
       % for other points, just start from previous configuration?
       [sol_hist, sol_opt_time] = meff_opt_single_point_func(q,p,v_vec(:,ii),pt_vec(:,ii),qsol_vec(:,ii-1));
   end
   
   % store outputs of optimizations   
   cost_vec(ii) = sol_hist(end,1);
   qsol_vec(:,ii) = sol_hist(end,3:5)';
   opt_time_vec(ii) = sol_opt_time;
   
   % animation at each point
   clf; hold on; 
   plot(pts_x,pts_y,'Xg','MarkerSize',8);
   [meff_temp, meffmin_temp] = plot_ik_sol_only(sol_hist,p,v_vec(:,ii),pt_vec(:,ii));
...
% plot of optimization outputs
figure; subplot(3,1,1);
plot(cost_vec); xlabel('Point #'); ylabel('Cost Function');
subplot(3,1,2);
plot(meff_vec); xlabel('Point #'); ylabel('Effective Mass');
subplot(3,1,3);
plot(opt_time_vec); xlabel('Point #'); ylabel('Optimization Time');
...
function [hist_start, hist_warm, opt_time] = meff_opt_single_point_warm_start_func(q,p,vdir,pt)
    ...
    cost_func2 = @(q)meff_cost(q,p,vdir,pt,q0);
...
%     plot_sols_and_optims(hist_warm,p,vdir,pt,'Cost: m_{eff}');
    ...
    function total_cost = meff_cost(q,p,vdir,rE,q0)
...
function [sol_hist, opt_time] = meff_opt_single_point_func(q,p,vdir,pt,q0)
    ...
    % run optimization
    cost_func = @(q)meff_cost(q,p,vdir,pt,q0);
    nonlcon = @(q)joint_constraints(q,p,vdir,pt,q0);
    A = []; % No other constraints
    b = [];
    Aeq = [];
    beq = [];
...
%     plot_sols_and_optims(sol_hist,p,vdir,pt,'Cost: m_{eff}');

    %%%%% helper functions %%%%%
    function [c,ceq] = joint_constraints(q,p,vdir,rE,q0)
...
function [meff1f, meffminf] = plot_ik_sol_only(sol_hist,p,vdir,rE)
