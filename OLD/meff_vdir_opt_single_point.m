...
% run optimization
q0 = inverse_kinematics(x,p,rE); % give a feasible initial solution
v0 = [1.0; 0.0]; % feasible initial velocity
x0 = [q0; v0]; % seed for optimization
cost_func = @(x)meff_cost(x,p,rE);
nonlcon = @(x)joint_constraints(x,p,rE);
...
tic;
[x_sol, fval_final, sol_hist] = fmincon_with_hist(cost_func,x0,A,b,Aeq,beq,lb,ub,nonlcon,options);
opt_time = toc;


% plot solution (evolution of solutions?)
fprintf('Solution: q = [%.2f, %.2f, %.2f], v = [%.2f, %.2f]\n',x_sol(1),x_sol(2),x_sol(3),x_sol(4),x_sol(5));
plot_sols_and_optims(sol_hist,p,rE,'Minimizing m_{eff} w.r.t. q and v');



%%%%% helper functions %%%%%
function [c,ceq] = joint_constraints(x,p,rE)
    
    % partition optimization variables
    q = x(1:3);
    v = x(4:5);
    ...
    fk_constraint = rE_temp(1:2)-rE;
    
    % need a unit velocity
    vel_constraint = v'*v - 1.0;
    
    % outputs
    c = [];
    ceq = [fk_constraint; vel_constraint]; 
    
end

function total_cost = meff_cost(x,p,rE)
    ...
    meff = 1/(v'*LLv_inv*v); % effective mass
        
    % minimize effective mass with velocity
    a1 = 1;
    c1 = a1*meff;
...
function [xsol, fval, history] = fmincon_with_hist(cost_func,x0,A,b,Aeq,beq,lb,ub,nonlcon_func,prev_options)
    ...
    function stop = save_output(x,optimvalues,state)
        stop = false;
        new_opt_data = [optimvalues.fval, optimvalues.constrviolation, x']; % for this use, new_opt_data = [cost, constraints, q1, q2, q3, vx, vy]
...
function q_ik = inverse_kinematics(x,p,rE)
...
function plot_sols_and_optims(sol_hist,p,rE,plt_title)
    ...
    v_iters = sol_hist(:,6:7)';
    fval_iters = sol_hist(:,1);
    constr_iters = sol_hist(:,2);
    num_pts = size(q_iters,2);
    
    % look at initial velocity direction too?
    vdir0 = v_iters(:,1);
    v_norm0 = vdir0/norm(vdir0);
    
    vdirf = v_iters(:,end);
    v_norm = vdirf/norm(vdirf); % make sure input velocity direction is a unit vector
