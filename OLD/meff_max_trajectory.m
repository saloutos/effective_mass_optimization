...
v_vec = zeros(2,num_pts);
meff_max_vec = zeros(1,num_pts);
qsol_max_vec = zeros(3,num_pts);
...
for ii=1:num_pts
   ...
   v_vec(:,ii) = vdir/norm(vdir);
   
   % determine initial configuration for optimization
   if ii==1
       % for first point, do optimization with warm start
       [sol_hist_start, sol_hist] = meff_opt_single_point_warm_start_func(q,p,v_vec(:,ii),pt_vec(:,ii));
   else
       % for other points, just start from previous configuration?
       sol_hist = meff_opt_single_point_func(q,p,v_vec(:,ii),pt_vec(:,ii),qsol_max_vec(:,ii-1));
   end
   
   % store outputs of optimizations   
   meff_max_vec(ii) = 1/sol_hist(end,1);
   qsol_max_vec(:,ii) = sol_hist(end,3:5)';
   
   % animation at each point
   clf; hold on; 
   plot(pts_x,pts_y,'Xg','MarkerSize',8);
   plot_ik_sol_only(sol_hist,p,v_vec(:,ii),pt_vec(:,ii));
   pause(0.5);
...
% wrap previous script in function here for easy use...this includes warm start and solves for the initial configuration with ik
function [hist_start, hist_warm] = meff_opt_single_point_warm_start_func(q,p,vdir,pt)
    ...
    cost_func1 = @(q)warm_start_max(q,p,vdir,pt);
    cost_func2 = @(q)meff_max_cost(q,p,vdir,pt);
...
%     plot_sols_and_optims(hist_warm,p,vdir,pt,'Cost: m_{eff}');
    ...
    function total_cost = warm_start_max(q,p,vdir,rE)

        th_link_3 = q(1)+q(2)+q(3); % angle between third link and world x-axis
        e_link_3 = [cos(th_link_3); sin(th_link_3)];

        % maximize dot product of e_link_3 and vdir
        a1 = 1;
        c1 = a1*(1/((e_link_3'*vdir)^2));
    ...
    function total_cost = meff_max_cost(q,p,vdir,rE)
        ...
        meff_inv = (vdir'*LLv_inv*vdir); % inverse of effective mass
        
        [V,D] = eig(LLv_inv);
        if D(1,1) > D(2,2) % direction of maximum effective mass
            meff_min_dir = V(:,1); 
        else
            meff_min_dir = V(:,2);
        end
        
        % maximize effective mass along vdir
        a1 = 1;
        c1 = a1*meff_inv; 
        
        % minimize dot product of minimum effective mass direction and vdir
        a2 = 0; %1;
        c2 = a2*(meff_min_dir'*vdir)^2;
...
% wrap previous script in function...no warm start in this one, but it does take a valid initial configuration
function sol_hist = meff_opt_single_point_func(q,p,vdir,pt,q0)
    ...
    % run optimization
    cost_func = @(q)meff_max_cost(q,p,vdir,pt);
    nonlcon = @(q)joint_constraints(q,p,vdir,pt);
    A = []; % No other constraints
    b = [];
    Aeq = [];
...
function plot_ik_sol_only(sol_hist,p,vdir,rE)
    ...
    % plot direction of lowest apparent inertia
    if D0(1,1) < D0(2,2)
        mm_dir0 = V0(:,1); % should already have unit magnitude
    else
        mm_dir0 = V0(:,2);         
    end
    % using parametric equations of ellipse, calculate ellipse points relative to foot position
    ...
    % plot direction of lowest apparent inertia
    if Df(1,1) < Df(2,2)
        mm_dirf = Vf(:,1); % should already have unit magnitude
        meffmmaxf = 1/Df(1,1);
    else
        mm_dirf = Vf(:,2);
        meffmmaxf = 1/Df(2,2);
    ...
    h_lm1 = quiver([rE(1)],[rE(2)],[mm_dirf(1)],[mm_dirf(2)],0.25,'r','LineWidth',2);
    h_lm2 = quiver([rE(1)],[rE(2)],[-mm_dirf(1)],[-mm_dirf(2)],0.25,'r','LineWidth',2);
    ...
    set(h_meff1f,'String',  sprintf('m_{eff,1} = %.2f',meff1f));
    set(h_meffminf,'String',  sprintf('m_{eff,max} = %.2f',meffmmaxf));

end

% for the trajectory of points, animate the solutions
