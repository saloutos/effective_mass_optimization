...
syms z [10 num_pts] real % [q_i[1:3]; dq_i[1:3]; u_i[1:3]; t] for each point, i
z = z(:); % vectorize

% initial values
z0 = zeros(10,num_pts);
z0(10,:) = linspace(0,Tf,num_pts); % start with normal time-steps
for ii=1:num_pts
...
% fmincon inputs
cost_func = @(z)meff_cost(z,p,pts,vels);
nonlcon = @(z)joint_constraints(z,p,pts,vels);
A = []; % no other linear inequalities
b = [];
% Aeq = zeros(10,num_pts); % constraints on initial and final positions?
% Aeq(10,1) = 1e6;
% Aeq = Aeq(:)';
% beq = [0];
...
[ub,lb] = joint_and_torque_limits(z,p,pts); % torque and joint velocity limits
...
z_sol = reshape(z_sol,10,num_pts);
...
u_sol = z_sol(7:9,:);
t_sol = z_sol(10,:);
t_sol(1) = 0; % doesn't affect anything in constraints or costs
...
%%%%% helper functions %%%%%
function [c,ceq] = joint_constraints(z,p,pts,vels)
    
%     num_pts = length(time_vec);
%     dt = time_vec(2)-time_vec(1);
    num_pts = size(pts,2);
    
    z = reshape(z,10,num_pts);
    q = z(1:3,:);
    dq = z(4:6,:);
    u = z(7:9,:);
    t = z(10,:);
...
%     ceq = [1e6*t(1)];
    ...
    for ii=2:num_pts
        % time(i-1)+dtmin < time(i)
        % time(i) < time(i-1)+dtmax
        c = [c; t(ii-1)+dtmin-t(ii); t(ii)-t(ii-1)-dtmax];
    end
    
    % dynamics constraints... using helper functions -> A*ddq = b -> x[i+1] = x[i] + dt*dx/dt[i]
    for ii=2:num_pts
        if ii==2
            dt = t(ii); % since t(1) should be 0
        else
            dt = t(ii)-t(ii-1);
        end
        z_i1 = [q(:,ii-1); dq(:,ii-1)];
    ...
    for ii=1:num_pts
        ...
        ceq = [ceq; 1e2*ceq_q_i];% ceq_dq_i];
    end
        
    % make magnitude of constraints larger to make sure they are satisfied?
    c = 1e2*c;
    ceq = 1e2*ceq;
    
end

function total_cost = meff_cost(z,p,pts,vels)
...
%         meff_cost_vec(ii) = -(ev_des'*LLv_inv*ev_des); % negative seems nicer to work with than inverse
        meff_cost_vec(ii) = -((ev_i'*LLv_inv*ev_i)^2); % negative seems nicer to work with than inverse
...
%         e3_cost_vec(ii) = (e_link_3'*ev_i)^2;
    ...
    R1 = eye(3);
    R1(1,1) = 0.1; % don't really care about joint 3 velocity
    R1(2,2) = -2.9;
    for ii=2:num_pts
        dq_i = dq(:,ii);
    ...
    % cost on inputs
    R2 = eye(3);
%     R2(1,1) = 0.1; % don't really care about joint 1 torque
    for ii=1:num_pts
        u_i = u(:,ii);
    ...
    % return total cost
    total_cost = c1 + c2 + c3 + c4 + c5 + c6 + c7 + c8 + c9 + c10;
    
end

function total_cost = meff_cost_OLD(z,p,pts,vels)
...
%     a1(1) = 3; % higher weight on effective mass at the start of the trajectory
    ...
    a3 = 0.1*ones(1,num_pts);
    c3 = a3*u_cost_vec;
    
    a4 = 0.1*ones(1,num_pts);
    c4 = 0; %a4*v_cost_vec;
    
    a5 = 0.1;
    c5 = a5*t(end); % minimize terminal time?
...
function [ub,lb] = joint_and_torque_limits(z,p,pts)
    ...
    for ii=1:num_pts
        ...
        ub = [ub; q_ub; dq_ub; u_ub; t_ub]; % format needs to match vectorized optimization variables
        lb = [lb; q_lb; dq_lb; u_lb; t_lb];        
